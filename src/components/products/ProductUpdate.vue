...
const emit = defineEmits(['patched', 'update:showModal', 'update:productId'])
...
const fetchProduct = async (id: number) => {
  const ps = await ProductService.getProduct(id)
  Object.assign(formData, ps)
}

const myProductIdRef = toRef(props, 'productId')

watch(myProductIdRef, async (newId: number) => {
  if (newId) {
    fetchProduct(newId)
  }
})
...
function aceptar() {
  ...
  patchProduct(props.productId, product)
  resetForm()
  changeModal.value = false
  changeProductId.value = 0
}
...
const changeProductId = computed({
  get: () => props.productId,
  set: (val: number) => emit('update:productId', val),
})
...
const patchProduct = async (id: number, product: CreateProduct) => {
  await ProductService.patchProduct(id, product)
  emit('patched')
}
</script>
  ...
  <div>
    ...
    >
      <div class="bg-white rounded-lg p-8 w-full max-w-xl shadow-lg relative">
        ...
        <div class="flex flex-col gap-y-5 w-full" v-if="formData.name !== undefined">
          <form>
            <input
              type="text"
              placeholder="Nombre"
              class="form-input rounded px-4 py-3 w-full mb-1 border border-gray-200"
            ...
            <input
              type="checkbox"
              class="form-checkbox rounded text-strong-blue"
